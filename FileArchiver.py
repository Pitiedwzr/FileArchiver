...
class MainWindow(QMainWindow):
    ...
    def enableLogin(self):
        config.common.skip_sign_in = False
        config.save()
        QMessageBox.information(
            None,
            QCoreApplication.translate("MainWindow", "Success"),
            QCoreApplication.translate("MainWindow", "Login will display on next startup.")
        )

    def disableLogin(self):
        config.common.skip_sign_in = True
        config.save()
        QMessageBox.information(
            None,
            QCoreApplication.translate("MainWindow", "Success"),
            QCoreApplication.translate("MainWindow", "Login won't display on next startup.")
        )
    ...
    def loadSnapshot(self):
        ...
        if len(snapshots) != 1: # Check if user choose more than one file
            QMessageBox.warning(
                ...
                QCoreApplication.translate("mainWindow", "You can only select one snapshot for loading.")
            )
            ...
            confirm = QMessageBox.question(
                ...
                QCoreApplication.translate(
                    "mainWindow", 
                    f"Are you sure you want to load the snapshot {snapshot_name}? This will undo all changes in that process."),
    ...
    def setupLanguageActions(self):
        ...
        for lang_code, (action, lang_name) in language_actions.items():
            # Use lambda function to avoid run it when connect it
            action.triggered.connect(lambda lc=lang_code, ln=lang_name: self.changeLanguage(lc, ln))
    ...
    def processCategory(self):
        ...
        except (NameError, AttributeError): # Check if user doesn't choose a path
...
if __name__ == "__main__":
    ...
    if locale != config.common.language and config.common.first_run:
    ...
    if config.common.first_run:
        config.common.first_run = False
        config.save()
    ...
    elif config.common.skip_sign_in:
        window.show()
